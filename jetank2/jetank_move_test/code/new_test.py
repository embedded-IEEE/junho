...
class JetankController(Node):
    def __init__(self):
        ...
        for i in range(1, 11):
            self.jenga_pubs[i] = {
                'attach': self.create_publisher(Empty, f'/jenga{i}/attach', qos_profile),
                'detach': self.create_publisher(Empty, f'/jenga{i}/detach', qos_profile)
            }
        ...
        self.MAGNET_FRAME = 'jetank/MAGNETIC_BAR_1' 
...
def main():
    ...
    try:
        ...
        robot.detach_all()
        time.sleep(1.0)
        robot.move_to_xyz(150, 0, 20, phi=-90, roll=0, move_time=2.0)
        time.sleep(3.0)
        print("=========================================================")
        ...
        while True:
            try:
                ...
                roll = vals[3] if len(vals) > 3 else 0.0
                
                phi = -90.0
                mv_time = 2.0
                
                print(f">> Sequence Start: ({x}, {y}, {z}) Roll={roll}")

                robot.move_to_xyz(x, y, z, phi=phi, roll=0, move_time=mv_time)
